...
?>
<div><h3><?php echo __('Manage BRP Year Make Model Search', 'brp-ymm-search'); ?></h3></div>
...
<div class="ymm-section">
  ...
  <form action="?page=ymm&action=importData" method="post" enctype="multipart/form-data">
      <fieldset class="ymm-fieldset">              
          ...
          &nbsp;&nbsp;
          <input name="submit" id="submit" class="button button-primary" value="<?php echo __('Import CSV', 'brp-ymm-search'); ?>" type="submit">                             
      </fieldset>
  </form>      
  <div><h4><?php echo __('Export YMM Data', 'brp-ymm-search'); ?>:</h4></div>     
  <form id="export_form" action="?page=ymm&action=exportData" method="post" enctype="multipart/form-data">
      <fieldset class="ymm-fieldset">
		  <?php wp_nonce_field('ymm_update_configuration', 'ymm_nonce'); ?>
          <input name="submit" id="submit" class="button button-primary" value="<?php echo $this->hasYmmData() ? __('Export CSV', 'brp-ymm-search') :  __('Export sample CSV', 'brp-ymm-search') ; ?>" type="submit">                                
      </fieldset>
  </form>
  <hr>  
  <div><h4><?php echo __('Configuration', 'brp-ymm-search'); ?>:</h4></div>
  <form id="export_form" action="?page=ymm&action=updateConfig" method="post" enctype="multipart/form-data">
      <fieldset class="ymm-fieldset">
          ...
          <br/><br/>
          <label for="ymm_excluded_category_ids"><?php echo __('Excluded Category IDs (comma-separated)', 'brp-ymm-search'); ?>:</label><br/>
          ...
          <br/><small><?php echo __('Enter the WooCommerce category IDs that should be excluded from the category dropdown, separated by commas. These categories will not appear in the search results.', 'brp-ymm-search'); ?></small>
          <br/><br/>                                                                                                                                                             
          <input name="submit" id="submit" class="button button-primary" value="<?php echo __('Save Configuration', 'brp-ymm-search') ; ?>" type="submit">      
      </fieldset>
  </form>     
</div>
