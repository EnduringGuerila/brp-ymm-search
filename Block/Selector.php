...
class Pektsekye_Ymm_Block_Selector {
  ...
  { 
    $values = $this->getSelectedValues();
    if (empty($values) || count($values) != $this->getLevelCount()){
      return '{}';
    }
    
    include_once( Pektsekye_YMM()->getPluginPath() . 'Controller/Selector.php');		
		$controller =  new Pektsekye_Ymm_Controller_Selector();  
    return $controller->getCategories($values, false); // this function returns JSON       
  } 
